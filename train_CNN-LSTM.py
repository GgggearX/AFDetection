...
def f1_score(y_true, y_pred):
    y_pred_binary = K.cast(K.greater(y_pred, 0.25), K.floatx())  # 进一步降低阈值以增加召回率
...
def main():
    parser = argparse.ArgumentParser(description='Train CNN-LSTM model for AF detection with k-fold CV')
    ...
    with h5py.File(args.path_to_hdf5, 'r') as f:
        x_data = f['tracings'][:]
        # 截断序列长度
        x_data = x_data[:, :args.max_seq_length, :]
...
# python train_CNN-LSTM.py data/ecg_tracings.hdf5 data/annotations/gold_standard.csv --n_splits 5 --epochs 250 --batch_size 32 --learning_rate 0.001 --max_seq_length 1024